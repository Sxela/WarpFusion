...
RUN python -m pip install requests mediapipe piexif safetensors lark Pillow==9.0.0 wget webdataset open_clip_torch opencv-python==4.5.5.64 pandas matplotlib fvcore ipywidgets==7.7.1 transformers==4.31.0 diffusers==0.11.1 omegaconf einops "pytorch_lightning>1.4.1,<=1.7.7" scikit-image opencv-python ai-tools cognitive-face zprint kornia==0.5.0 lpips keras datetime timm==0.6.7 prettytable basicsr fairscale realesrgan torchmetrics==0.11.4
WORKDIR /content
...
RUN git clone https://github.com/Sxela/Segment-and-Track-Anything-CLI /content/Segment-and-Track-Anything-CLI && python -m pip install -e /content/Segment-and-Track-Anything-CLI/sam
RUN git clone https://github.com/Sxela/ControlNet-v1-1-nightly /content/ControlNet
RUN python -m pip install -e git+https://github.com/CompVis/taming-transformers.git@master#egg=taming-transformers -e git+https://github.com/openai/CLIP.git@main#egg=clip -e git+https://github.com/IDEA-Research/GroundingDINO.git@main#egg=GroundingDINO
